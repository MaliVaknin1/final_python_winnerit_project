...
class LoginPage(BasePage):
    ...
    def validate_success_login(self, username: str, passwords: str):
        with allure.step(f"validating success login with username:{username} and password:{passwords}"):
    ...
    def fail_login_without_password(self):
        with allure.step(f"validating failed login without password, error message is:{ERRORS.EMPTY_PASSWORD_FIELD_ERROR.value}"):
    ...
    def fail_login_locked_out_user(self):
        with allure.step(f"validating failed login with locked out user, error message is:{ERRORS.LOCKED_OUT_USER_ERROR.value}"):
    ...
    def fail_login_incorrect_user(self):
        with allure.step(f"validating failed login with incorrect user, error message is:{ERRORS.INCORRECT_USERNAME_ERROR.value}"):
    ...
    def fail_login_empty_user_and_pass(self):
        with allure.step(f"validating failed login without username and password, error message is:{ERRORS.EMPTY_USERNAME_FIELD_ERROR.value}"):
